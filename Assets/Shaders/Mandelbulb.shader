Shader "Custom/Mandelbulb" {
	Properties{
		...
		_N("N", Range(-100,100)) = 1.0
		_R2("R2", Range(-100,100)) = 1.0
			...
			// float _OffsetVal;
			 ...
			 void vert(inout appdata_full v, out Input o) {
				 ...
				 // o.vertColor = v.color;
				  ...
				  float theta = atan2(sqrt(v.vertex.x * v.vertex.x + v.vertex.y * v.vertex.y), v.vertex.z);//theta = r == 0.0 ? 0.0 : asin(v.vertex.z / r);
				  float radius = GetRadius(phi, theta, _Time);
				  float r2 = sqrt(v.vertex.x * v.vertex.x + v.vertex.y * v.vertex.y + v.vertex.z * v.vertex.z);
				  ...
				  v.vertex.x =  pow(r2, _N)  * sin(theta * _N) * cos(phi * _N); //* radius;
				  v.vertex.y =  pow(r2 , _N)  * sin(theta * _N) * sin(phi * _N); //* radius;
				  v.vertex.z = pow(r2, _N)  * cos(theta * _N); // * radius;
