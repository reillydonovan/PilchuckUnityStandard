...
public class SinusWave
{
    ...
    public double calculateSignalValue(double newSignalTime, double newSignalFrequency, double newSignalAmplitude, double stepPercentage)
    {
        currentSignalAmplitude = newSignalAmplitude * stepPercentage + currentSignalAmplitude * (1.0 - stepPercentage);
        if (currentSignalFrequency != newSignalFrequency)
        {
            ...
            currentSignaOutlValue = currentSignalAmplitude * Math.Sin(currentSignalTime * 2.0 * Math.PI * currentSignalFrequency + currentSignalPhaseOffset);
