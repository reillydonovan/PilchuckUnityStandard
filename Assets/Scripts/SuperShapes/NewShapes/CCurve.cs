...
public class CCurve : MonoBehaviour
{
    ...
    Mesh UpdateMesh(Mesh m)
    {
        ...
        float radsPerPhiDiv = 21 * Mathf.PI / (phiDivs - 1);
           ...
           // u = Remap(i, 0, phiDivs - 1, 0.5f, 1.0f);
            ...
            for (int j = 0; j < thetaDivs; j++)
            {
             ...
             //   v = Remap(j, 0, thetaDivs, 0, 2 * Mathf.PI);
                ...
                r = 5 * (1 + Mathf.Sin(11 * u / 5)) - 4 * Mathf.Pow(Mathf.Sin(17 * u / 3), 4) * Mathf.Pow(2 * Mathf.Cos(3 * u) - 28 * u, 8); 
