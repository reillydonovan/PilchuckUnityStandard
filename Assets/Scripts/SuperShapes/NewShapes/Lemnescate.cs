...
public class Lemnescate : MonoBehaviour
{
    ...
    Mesh UpdateMesh(Mesh m)
    {
        ...
        for (int i = 0; i < phiDivs; i++)
        {
            ...
            for (int j = 0; j < thetaDivs; j++)
            {
                ...
                // u = phi;
                //v = theta;
                 v = Remap(j, 0, thetaDivs, 0, Mathf.PI);
               ...
               // y = cos(v) sqrt(| sin(2 u) |) sin(u)
                ...
                x = Mathf.Cos(u) * Mathf.Sqrt(Mathf.Abs(Mathf.Sin(2 * v))) * Mathf.Cos(v);
                y = Mathf.Cos(u) * Mathf.Sqrt(Mathf.Abs(Mathf.Sin(2 * v))) * Mathf.Sin(v);
                z = Mathf.Pow(x, 2) - Mathf.Pow(y, 2) + 2 * x * y * Mathf.Pow(Mathf.Tan(u), 2);
